...
<template>
  <div class="toolbar filter-toolbar gap-3 py-2.5" aria-label="Фильтры материалов">
    ...
    <label v-if="!documents" class="field-label">Категория / тег<select class="form-control" :value="modelValue.tag?.[0] ?? ''" @change="set('tag', value($event) ? [value($event)] : undefined)"><option value="">Все категории</option><option v-for="tag in [...new Set([...(filters?.categories ?? CATEGORIES), ...(filters?.tags ?? [])])]" :key="tag">{{ tag }}</option></select></label>
